...
SFlash_StatusTypeDef EE::Format(uint32_t addr)
{
	HAL_FLASH_Unlock();
...
SFlash_StatusTypeDef EE::Write(uint32_t address, void *data, size_t sizeofDataType){
...
SFlash_StatusTypeDef EE::Read(uint32_t address, void *data, size_t sizeOfDataType){
	static uint8_t lengh = 0;
...
SFlash_StatusTypeDef EE::Update(uint32_t address, void *data, size_t sizeOfDataType){
	...
	case STATE_PAGE_CLONE:
		while(EE::Read(FLASH_ADDR_BASE, &clone, sizeof(flashPIDParam)) != SFLASH_OK);
	...
	case STATE_PAGE_ERASED:
		EE::Format(FLASH_ADDR_BASE);
		sflashprocc = STATE_PAGE_WRITE;
		break;
	case STATE_PAGE_WRITE:
		while(EE::Write(FLASH_ADDR_BASE, &clone, sizeof(flashPIDParam))!= SFLASH_OK);
